...
import { formatCurrency, getStatusTone, getCallTone } from '../utils.js';
  ...
  return html`
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        ${['All', 'Pending', 'Confirmed', 'Cancelled', 'Failed'].map((item) => html`
          <button
            key=${item}
        ...
        <div className="hidden overflow-auto rounded-[var(--radius-lg)] border border-[hsl(var(--border))] xl:block">
          <table className="w-full min-w-[1180px] divide-y divide-[hsl(var(--border))]">
            ...
            <thead className="bg-[hsl(var(--muted)/0.65)]">
              <tr>
                ${['Sr. No.', 'Order', 'Customer', 'Product', 'Phone', 'Total', 'Status', 'Call', 'Retry', 'Actions'].map((head) => html`
                  <th
                    key=${head}
